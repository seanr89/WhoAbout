...
public class BookingService
{
    ...
    public async Task<Booking> CreateBookingAsync(Booking booking)
    {
        ...
        // Check if staff member has a reserved desk
        ...
        {
             // Check if the reserved desk is released for this day
             var isReleased = await _context.DeskReleases
                .AnyAsync(r => r.DeskId == reservedDesk.Id && r.Date == startOfDay);

             if (!isReleased)
             {
                 throw new Exception("You have a reserved desk that is not released for this date.");
             }
        }
